...
def GUIreceive():
    ...
    def ablesen():
        ...
        COMPORT1 = clicked.get()
        COMPORT2 = COMPORT1.replace(" ", "")
        COMPORT3 = COMPORT2.replace("[S]", "")
        COMPORT4 = COMPORT3.replace("[R]", "")


        #Connecting to serial port chosen by the user and reading the last three lines (must be three to work with the .ino)
        arduino = serial.Serial(COMPORT4, baudrate = 115200)
        ...
        #Displaying empty label to prevent overwriting of old values
    ...
    while i <= len(port) and i < numberofports:
        ...
        arduino1 = serial.Serial(port[i], baudrate = 115200, timeout=2)

        arduino_bytes4 = str(arduino1.readline())
        arduino_bytes5 = str(arduino1.readline())
        arduino_bytes6 = str(arduino1.readline())

        #Checking if the char sequence "EddystoneURL" or "I am available" can be found and storing it for further processing(should be in exactly one of the three provided strings)
        
        if "EddystoneURL" in arduino_bytes4:
            print("Found receiver at: " + (port[i]))  
            Receiver = (port[i]) +" [R]"
            Receiverc = (port[i])
            print("Receiver: " + Receiver)
            options.append(Receiver)

        elif "EddystoneURL" in arduino_bytes5:
            print("Found receiver at: " + (port[i]))
            Receiver = (port[i]) +" [R]"
            Receiverc = (port[i])
            print("Receiver: " + Receiver)
        ...
        elif "EddystoneURL" in arduino_bytes6:
            print("Found receiver at: " + (port[i]))
            Receiver = (port[i]) +" [R]"
            Receiverc = (port[i])
            print("Receiver: " + Receiver)
        ...
        elif "I am available!" in arduino_bytes4:
            print("Found Sender at: " + (port[i]))
            Sender = (port[i]) +" [S]"
            Senderc = (port[i])
            print("Sender: " + Sender)
            options.append(Sender)

        elif "I am available!" in arduino_bytes5:
            print("Found Sender at: " + (port[i]))
            Sender = (port[i]) +" [S]"
            Senderc = (port[i])
            print("Sender: " + Sender)
        ...
        elif "I am available!" in arduino_bytes6:
            print("Found Sender at: " + (port[i]))
            Sender = (port[i]) +" [S]"
            Senderc = (port[i])
            print("Sender: " + Sender)
    ...
    #Adding another entry to the list, ensuring every option is displayed at all times
        arduino1.close()
    if (port[0] == Senderc):
        options.append(Sender)
    elif (port[0] == Receiverc):
        options.append(Receiver)
    else:
...
def GUIsend():
    ...
    def submit():
        ...
        #Fetching port chosen by the user and editing it
        COMPORT1 = clicked.get()
        COMPORT2 = COMPORT1.replace(" ", "")
        COMPORT3 = COMPORT2.replace("[S]", "")
        COMPORT4 = COMPORT3.replace("[R]", "")
        ...
        if(ZVint < 121 and ZDint < 1001 and GKSint < 1024 and ZVint > -1 and ZDint > -1 and GKSint > -1): 
            ...
            msg = Message(rootS, text = "                        ", font=('calibre', 10), width=200).grid(row=8, column=1)
...
#Made by Noah at BBR - 11.05.2022